...
export default class ViewPage extends CommonActions {
   ...
   */
  async verifyDetails(
    expectedData: ViewDetails,
    audioFilePath: string,
  ): Promise<void> {
    this.logger.info(`[ViewPage] Verifying details for claim: ${expectedData.ClaimNumber}`);
    // Verify Claim Number
    ...
    this.logger.info(`[ViewPage] Verifying Claim Number: ${expectedData.ClaimNumber}`);
    const claimNumberLocator: Locator = this.locators
      .claimNumberByText(expectedData.ClaimNumber)
      .first();
    ...
    this.logger.info(`[ViewPage] Verifying Insured Name: ${fullInsuredName}`);
    await this.ASSERT.hardAssert(
      this.locators.insuredValue,
      AssertionType.TO_HAVE_TEXT,
      fullInsuredName,
    ...
    this.logger.info(`[ViewPage] Verifying Interviewee Name: ${fullIntervieweeName}`);
    await this.ASSERT.hardAssert(
      this.locators.intervieweeValue,
      AssertionType.TO_HAVE_TEXT,
      fullIntervieweeName,
    ...
    this.logger.info(`[ViewPage] Verifying Interviewee Type`);
    await this.ASSERT.hardAssert(
      this.locators.intervieweeTypeValue,
      AssertionType.TO_HAVE_TEXT,
      `Witness`,
      "'Interviewee Type' should match",
    );

    // Verify Date of Loss (DOI)
    this.logger.info(`[ViewPage] Verifying Date of Loss: ${expectedData.DOL}`);
    await this.ASSERT.hardAssert(
      this.locators.dateOfIncidentValue,
      AssertionType.TO_HAVE_TEXT,
      expectedData.DOL,
      "'Date of Loss' should match",
    );

    // Verify Adjuster
    this.logger.info(`[ViewPage] Verifying Adjuster: ${expectedData.Adjuster}`);
    await this.ASSERT.hardAssert(
      this.locators.adjusterValue,
      AssertionType.TO_HAVE_TEXT,
      expectedData.Adjuster,
      "'Adjuster' should match",
    );

    // Verify Office
    this.logger.info(`[ViewPage] Verifying Office: ${expectedData.Office}`);
    await this.ASSERT.hardAssert(
      this.locators.officeValue,
      AssertionType.TO_HAVE_TEXT,
      expectedData.Office,
      "'Office' should match",
    );
    ...
    // Verify Audio Length
    this.logger.info(`[ViewPage] Verifying Audio Length`);
    await this.verifyAudioDuration(
      audioFilePath,
      this.locators.audioDurationText,
    );
  }
   ...
   * @private
   */
  private async verifyAudioDuration(
    filePath: string,
    durationElement: Locator,
  ): Promise<void> {
    this.logger.info(`[ViewPage] Verifying audio duration for: ${filePath}`);

    const expectedDurationSeconds =
      await this.getExpectedAudioDuration(filePath);
    ...
    this.logger.info(`[ViewPage] Expected Duration: ${expectedDurationDisplay}`);

    const actualDurationText = await durationElement.innerText();
    this.logger.info(`[ViewPage] Actual UI Duration: ${actualDurationText}`);

    await this.ASSERT.hardAssertValue(
      actualDurationText,
      ValueAssertionType.TO_BE,
      expectedDurationDisplay,
      "Audio file length should match the UI display",
    );
    this.logger.info(`[ViewPage] 🎉 Verification successful! UI duration matches file duration.`);
  }
  ...
  private async getExpectedAudioDuration(filePath: string): Promise<number> {
    this.logger.info(`[ViewPage] Getting expected audio duration for: ${filePath}`);
    try {
      const duration = await getAudioDurationInSeconds(filePath);
      this.logger.info(`[ViewPage] Successfully retrieved audio duration: ${duration} seconds.`);
      return duration;
    } catch (error) {
      this.logger.error(`[ViewPage] Error getting audio duration for ${filePath}:`, error);
      throw new Error(
        `Failed to determine expected audio duration. Original error: ${error.message}`,
      );
    }
  }
}
